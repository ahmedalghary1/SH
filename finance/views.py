...
from config.date_ranges import PERIOD_CHOICES, filter_by_split_datetime
...
class TransactionListView(ManagerRequiredMixin, ExportListMixin, ListView):
    ...
    def get_queryset(self):
        ...
        qs, self.date_filter = filter_by_split_datetime(qs, self.request.GET, 'transaction_date', 'transaction_time')
        return qs
...
class CustomerCollectionView(RoleRequiredMixin, FormView):
    ...
    def get_collection_queryset(self):
        ...
        queryset, self.date_filter = filter_by_split_datetime(queryset, self.request.GET, 'transaction_date', 'transaction_time')
        if query:
